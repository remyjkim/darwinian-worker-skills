...
2. Update version fields in `.claude-plugin/plugin.json`,
   ...
   `package.json`.
3. Update `bundle.json` so `bundleName`, `version`, and skill entries match
   the package.
4. Open a release PR.
5. On merge to `main`, `create-tag.yml` tags `v<version>`.
6. `publish-release.yml` creates a GitHub Release.
...
- `drwn library add skill . --json` and `drwn skills packages show darwinian-harness-skills --json` from a disposable `HOME`
- Smoke-test at least one skill in Claude Code
...
## Coordinating With darwinian-harness
...
- Safety gaps: file them in `darwinian-harness`; do not silently paper over
  them here.
